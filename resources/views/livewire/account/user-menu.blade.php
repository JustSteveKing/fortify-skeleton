<div x-data="{
    ...
    <div x-show="dropdownOpen"
        ...
        <div class="p-1 mt-1 bg-slate-100 dark:bg-slate-800 border rounded-md shadow-md border-slate-200/70 dark:border-slate-200/70">
            <div class="px-2 py-1.5 text-sm font-semibold">User Account</div>
            ...
            <div class="px-2 py-1.5 text-sm font-semibold">Team Account</div>
            @foreach ($this->user->memberships as $membership)
                <a wire:navigate href="{{ route('pages:accounts:show', $membership->account) }}" class="relative flex cursor-pointer select-none hover:bg-slate-200 dark:hover:bg-slate-700 items-center rounded px-2 py-1.5 text-sm outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50">
                    <x-icons.company class="w-4 h-4 mr-2" />
                    <span>{{ $membership->account->name }}</span>
                </a>
            @endforeach
            <a wire:navigate href="{{ route('pages:accounts:create') }}" class="relative flex cursor-pointer select-none hover:bg-slate-200 dark:hover:bg-slate-700 items-center rounded px-2 py-1.5 text-sm outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50">
